...
class RmiModel:
    ...
    def build(self):
        ...
        for i in range(self.level[2]):
            temp_model = LinearRegression()
            self.model[2].append(temp_model)
            ...
            if m == 1 :
                ...
                if m != self.level[-1]:
                    ...
                    for i in range(self.data_size):
                        ...
                        sub_y[mm].append(y[i])
                    for k in range(m):
                        xx = np.array(sub_data[k])
                        yy = np.array(sub_y[k])
                        self.model[1][k].fit(xx,yy)
                # 训练下层 模型
                if m==self.level[-1] :
                    above_level = self.level[-2]
                    subsub_data = [[] for _ in range(m)]
                    subsub_y = [[] for _ in range(m)]
                    for index in range(above_level):
                        pass
                    for j in range(m):
                        print("    训练下层model", j)
